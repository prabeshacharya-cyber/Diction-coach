...
function deepinfraKey(): string {
  ...
  if (!k) throw new Error("DEEPINFRA_API_KEY is not set. Add it in your environment secrets.");
  return k;
}
...
async function deepinfraChat(
  ...
  if (!resp.ok) {
    ...
    throw new Error(`DeepInfra error (${resp.status}): ${err}`);
  }
...
// ---------------------------------------------------------------------------
// Model catalog  (DeepSeek V3 = default, cheapest capable model)
// ---------------------------------------------------------------------------
export const MODELS = [
  { id: "deepseek-ai/DeepSeek-V3", name: "DeepSeek V3 (Default)", group: "DeepSeek" },
...
const DEFAULT_MODEL_ID = "deepseek-ai/DeepSeek-V3";

// Vision model used for body language analysis
...
router.post("/transcribe", async (req, res) => {
  ...
  try {
    ...
    const [transcript, bodyLanguageAnalysis] = await Promise.all([
      transcribeAudio(audioFile),
      videoFrames && videoFrames.length > 0
        ? analyzeBodyLanguage(videoFrames).catch((err) => {
            console.warn("Body language analysis skipped:", err);
            return undefined;
          })
...
async function transcribeAudio(file: File): Promise<string> {
  try {
    ...
    // Fall back to OpenAI transcription if DeepInfra fails
    console.warn("DeepInfra Whisper failed, falling back to OpenAI:", err);
    const { openai } = await import("@workspace/integrations-openai-ai-server/audio");
...
// ---------------------------------------------------------------------------
// POST /evaluate  — DeepInfra primary, Gemini special case
// ---------------------------------------------------------------------------
router.post("/evaluate", async (req, res) => {
  ...
  try {
    ...
    if (chosenModel.startsWith("gemini/")) {
      ...
      // All other models go through DeepInfra (DeepSeek, Llama, Qwen, Mixtral, etc.)
      feedback = await deepinfraChat(
        chosenModel,
        [
...
// ---------------------------------------------------------------------------
// POST /speak — Kokoro-82M (DeepInfra), falls back to OpenAI TTS
// ---------------------------------------------------------------------------
...
async function generateSpeech(text: string): Promise<Buffer> {
  try {
    ...
    if (!resp.ok) throw new Error(`Kokoro error ${resp.status}: ${await resp.text()}`);
